...
const AdminPage = () => {
  ...
  // Modals state
  // activeModal can be: null, 'add', 'edit', 'restock'
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-6 border-b border-gray-200">
          ...
          <button
            ...
            className="mt-4 md:mt-0 inline-flex items-center justify-center px-5 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 hover:scale-[1.01] active:scale-[0.99] transition duration-150"
          >
        ...
        {successMessage && (
          <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-lg flex items-start space-x-3 text-emerald-800 animate-fade-in-down">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Unique Models</p>
              <h3 className="text-2xl font-bold text-gray-950 mt-1">{loading ? '...' : totalVehicles}</h3>
            </div>
            <span className="p-3 bg-indigo-50 text-indigo-650 rounded-lg">
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-950 mt-1">{loading ? '...' : totalStock}</h3>
            </div>
            <span className="p-3 bg-emerald-50 text-emerald-650 rounded-lg">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className={`text-2xl font-bold mt-1 ${outOfStockCount > 0 ? 'text-red-650' : 'text-gray-950'}`}>
                {loading ? '...' : outOfStockCount}
              </h3>
            </div>
            <span className={`p-3 rounded-lg ${outOfStockCount > 0 ? 'bg-red-50 text-red-650' : 'bg-gray-50 text-gray-500'}`}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <h3 className="mt-4 text-lg font-bold text-gray-900">Inventory is empty</h3>
            <p className="mt-2 text-sm text-gray-500">Get started by creating your first car model.</p>
            <button
              onClick={openAddModal}
              className="mt-6 inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-indigo-650 hover:bg-indigo-700 transition"
            >
              Add Vehicle
            </button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Make & Model</th>
                    <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Category</th>
                    <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Price</th>
                    <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Quantity</th>
                ...
                <tbody className="bg-white divide-y divide-gray-150">
                  {vehicles.map((vehicle) => (
                    <tr key={vehicle._id} className="hover:bg-gray-50 transition duration-100">
                      <td className="px-6 py-4.5 whitespace-nowrap">
                        <div className="text-sm font-bold text-gray-900">{vehicle.make}</div>
                      ...
                      <td className="px-6 py-4.5 whitespace-nowrap text-sm font-bold text-gray-900">
                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(vehicle.price)}
                      </td>
                      <td className="px-6 py-4.5 whitespace-nowrap">
                        {vehicle.quantity === 0 ? (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-50 text-red-750">
                            Out of Stock
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4.5 whitespace-nowrap text-right text-sm font-medium space-x-3">
                        ...
                        <button
                          ...
                          className="inline-flex items-center text-emerald-650 hover:text-emerald-800 transition"
                          title="Restock"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center text-indigo-650 hover:text-indigo-850 transition"
                          title="Edit"
                        >
                          <svg className="w-5 h-5 mr-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                          ...
                          className="inline-flex items-center text-red-650 hover:text-red-800 transition"
                          title="Delete"
                        >
                          <svg className="w-5 h-5 mr-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {(activeModal === 'add' || activeModal === 'edit') && (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-40 flex items-center justify-center p-4">
            <div className="relative bg-white w-full max-w-xl rounded-xl shadow-xl border border-gray-150 overflow-hidden transform transition-all">
              
              {/* Form submit error badge inside modal */}
              {formSubmitError && (
                <div className="bg-red-50 text-red-700 text-sm font-semibold p-4 border-b border-red-150 flex items-center gap-2">
        ...
        {activeModal === 'restock' && (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-40 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-xl shadow-xl border border-gray-150 p-6 relative">
              <h3 className="text-xl font-bold text-gray-950 mb-3 border-b border-gray-100 pb-3">
                Restock Inventory
              </h3>
              
              {formSubmitError && (
                <div className="mb-4 p-3 bg-red-50 text-red-700 rounded-lg text-sm border border-red-150 flex items-center gap-2 font-medium">
                  <svg className="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span>{formSubmitError}</span>
              ...
              <p className="text-sm text-gray-600 mb-4">
                How many units of <strong className="text-gray-900">{selectedVehicle?.make} {selectedVehicle?.model}</strong> do you want to add to the inventory? (Current stock: {selectedVehicle?.quantity})
              </p>

              <form onSubmit={handleRestockSubmit}>
                <div className="mb-5 flex flex-col">
                  <label className="text-xs font-semibold text-gray-500 mb-1">Restock Amount</label>
                  <input
                    type="number"
                    min="1"
                    step="1"
